...
class CameraManager:
    """
    Reliable camera wrapper with flexible camera class.
    Supports RealSenseD415 with manual filters/exposure.
    """
...
class PoseRecorder:
    ...
    drag: bool = False
    depth_filter: any = field(default_factory=lambda: None)
    ...
    def _save_frames(self, idx: str, color: np.ndarray, depth: np.ndarray) -> None:
        filtered_depth = (
            self.depth_filter.filter(depth) if self.depth_filter is not None else depth
        )
...
def _add_record_args(parser):
    ...
    parser.add_argument(
        "--drag", action="store_true", help="Enable drag teaching mode while recording"
    )
    parser.add_argument(
        "--db_path", default="robot_data.lmdb", help="LMDB database path"
    )
    parser.add_argument(
        "--use-db", action="store_true", help="Use LMDB storage instead of JSON"
    )
...
def load_trajectory_db(
    ...
    """Retrieve trajectory poses from an :class:`LmdbStorage` backend."""
...
def _add_record_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--use-db",
        action="store_true",
        help="Use LMDB storage instead of JSON",
    )

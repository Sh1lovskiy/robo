...
# === Paths ===
colmap_data_dir = "/home/sha/Documents/work/robohand_v2/colmap/data"
...
with open(left_params_txt, "w") as f: f.write(left_params_str)
with open(right_params_txt, "w") as f: f.write(right_params_str)
...
for pose in camera_poses:
    base_name = pose["frame"][:-4]  # Remove .png from frame_XXX.png
    for folder, suffix in [(left_images_folder, "_left.png"), (right_images_folder, "_right.png")]:
        img_path = os.path.join(folder, base_name + suffix)
        if not os.path.exists(img_path):
            raise FileNotFoundError(f"Missing image: {img_path}")
        img_check = cv2.imread(img_path)
        if img_check.shape[:2] != (img_h, img_w):
            raise ValueError(f"Image {img_path} has unexpected size {img_check.shape[:2]}")
        image_count += 1
...
with open(cameras_txt_path, "w") as f:
    ...
    f.write(f"1 PINHOLE {img_w} {img_h} {left_K[0,0]} {left_K[1,1]} {left_K[0,2]} {left_K[1,2]}\n")
    f.write(f"2 PINHOLE {img_w} {img_h} {right_K[0,0]} {right_K[1,1]} {right_K[0,2]} {right_K[1,2]}\n")
...
# === Write images.txt with relative paths ===
with open(images_txt_path, "w") as f:
    f.write("# Image list with two lines of data per image:\n")
    f.write("# IMAGE_ID, QW, QX, QY, QZ, TX, TY, TZ, CAMERA_ID, NAME\n")
    f.write("# POINTS2D[] as (X, Y, POINT3D_ID)\n")
    for idx, pose in enumerate(camera_poses):
        base = pose["frame"][:-4]  # Remove .png from frame_XXX.png
        rx, ry, rz = pose["camera_pose"]["world_orientation"]
        tx, ty, tz = pose["camera_pose"]["world_position"]
        ...
        left_img_path = os.path.join(left_images_folder, f"{base}_left.png")
        if not os.path.exists(left_img_path):
            raise FileNotFoundError(f"Image not found: {left_img_path}")
        rel_left_path = os.path.relpath(left_img_path, os.path.join(colmap_data_dir, "images"))
        f.write(f"{idx*2+1} {q_left[0]} {q_left[1]} {q_left[2]} {q_left[3]} {t_left[0]} {t_left[1]} {t_left[2]} 1 {rel_left_path}\n\n")

        # Right camera
        R_left = T_left2world[:3, :3]
        t_left = T_left2world[:3, 3]
        R_right = R_left @ R_rel
        t_right = t_left + R_left @ T_rel[:, 0]
        T_right2world = np.eye(4)
        T_right2world[:3, :3] = R_right
        T_right2world[:3, 3] = t_right
        q_right, t_right = matrix_to_quat_T(T_right2world)
        right_img_path = os.path.join(right_images_folder, f"{base}_right.png")
        if not os.path.exists(right_img_path):
            raise FileNotFoundError(f"Image not found: {right_img_path}")
        rel_right_path = os.path.relpath(right_img_path, os.path.join(colmap_data_dir, "images"))
        f.write(f"{idx*2+2} {q_right[0]} {q_right[1]} {q_right[2]} {q_right[3]} {t_right[0]} {t_right[1]} {t_right[2]} 2 {rel_right_path}\n\n")

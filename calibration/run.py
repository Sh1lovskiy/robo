...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--save-images", action="store_true", help="Save overlay images"
    )
    parser.add_argument(
        "--interactive", action="store_true", help="Confirm steps via keyboard"
    )
    parser.add_argument(
        "--max-frames", type=int, default=20, help="Maximum frames to capture"
    )
    return parser
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.capture_poses:
        ...
        capture_dataset(
            out_dir, max_frames=args.max_frames, interactive=args.interactive
        )
        image_dir = out_dir
